...
function materialWaste(totalMaterial, materialUnits, numJobs, jobConsumption) {
    let s2_1 = numJobs * jobConsumption
    let s2_2 = totalMaterial - s2_1
    return(s2_2 + materialUnits)
}
...
function interest(principal, rate, periods) {
    let s3_1 = rate * periods
    let s3_2 = principal * s3_1
    let s3_3 = principal + s3_2
    return Math.floor(s3_3)
}
